    ...
    const interval = setInterval(() => {
      if(provider) {
        httpWeb3.eth.getBalance(walletAddress).then((balance) => {
          const value = Web3.utils.fromWei(balance.toString(), "ether");
          if (isSubscribed) {
        ...
        avaxGeishaContract.methods.viewUserReward(walletAddress).call().then((result: any) => {
          if (isSubscribed) {
            const reward = Web3.utils.fromWei(result.toString(), "ether");
              ...
              <Link
                  ...
                  onClick={async () => {
                    ...
                    if(provider) {
                      ...
                      const owner = avaxGeishaContract.methods.owner().call();
                      if(owner.toString().toUpperCase() === walletAddress.toUpperCase()) { // owner
                        toast('danger', `Onwer can't call claimUserReward(), please user withdrawFunds() function.`);
                        return;
                      }
